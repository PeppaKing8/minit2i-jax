...
def get_config():
  ...
  # Distillation dataset options.
  dataset.use_cc12m = False
  dataset.use_laion_aes = False
  dataset.use_blip3_short = False
  dataset.use_blip3_long = False
  dataset.use_blip3_journey = False

  # Optional high-quality roots are kept for experiments, but the released
  # MF recipe uses CC12M by default.
  ...
  dataset.laion_aes_root = settings.LAION_AES_ROOT
  dataset.blip3_short_root = settings.BLIP3_SHORT_ROOT
  dataset.blip3_long_root = settings.BLIP3_LONG_ROOT
  dataset.blip3_journey_root = settings.BLIP3_JOURNEY_ROOT
  ...
  dataset.llm = 'google/flan-t5-base'
  ...
  training.seed = 42
  training.ema_type = 'edm'
  training.ema_vals = [2000]  # EDM halflife in kimg; support multiple values.
  ...
  config.sampling_pmf = sampling_pmf = ml_collections.ConfigDict()
  sampling.seed = 0
  sampling.prediction = 'v'  # 'v' or 'x'
  sampling.sampler = 'euler'  # euler-4e-2, euler, heun, sde
  sampling.a_min = 0.01  # floor for clip(1-t) in x->v conversion
  sampling.label_drop_rate = 0.1
  sampling.t_sample_schedule = 'uniform'  # uniform, lognorm
  sampling.t_lognorm_mu = 0.0
  sampling.t_lognorm_sigma = 1.0
  sampling.noise_scale = 2.0
  sampling_pmf.P_mean = 0.8
  sampling_pmf.P_std = 0.8
  sampling_pmf.noise_scale = 2.0
  sampling_pmf.cfg_beta = 1.0
  sampling_pmf.cfg_max = 12.0
  sampling_pmf.data_proportion = 0.5
  sampling_pmf.norm_p = 1.0
  sampling_pmf.norm_eps = 0.01
  sampling_pmf.adapt_weight = True
  ...
  eval_cfg.cfg_scale = 2.0
  eval_cfg.include_ema = False
  eval_cfg.num_steps = 4
  eval_cfg.t_min = 0.2
  eval_cfg.t_max = 0.8

  eval_cfg.mjhq = mjhq = ml_collections.ConfigDict()
  mjhq.enable = False
  mjhq.stats_cache = settings.MJHQ_FID_STATS  # required when enable=True
  mjhq.caption_file = settings.MJHQ_CAPTION_FILE  # required when enable=True
  ...
  config.load_from = ''
  config.load_pt_from = ''
  config.student_from_scratch = False
  config.eval_only = False
  config.eval_show_sample = False
  config.mean_flow_distill = False
