...
def get_fid_evaluator(workdir, config, p_sample_step, run_p_sample_step, mesh_bundle=None):
    ...
    target. Returns None if no FID datasets are enabled.
    ...
    if getattr(config.eval.mscoco, "enable", False):
        eval_targets.append((
            "mscoco",
            config.eval.mscoco,
            config.eval.mscoco.stats_cache,
            config.eval.mscoco.caption_file or MSCOCO_CAPTION_FILE,
        ))
    if not eval_targets:
        return None
    ...
    def evaluator(params, step, writer, cfg_scale=1.0, descriptor=""):
        ...
        metrics = {}
        vis_samples = None
        for dataset, target_cfg, cache_path, caption_file in eval_targets:
            target_cfg_scale = getattr(target_cfg, "cfg_scale", cfg_scale)
            if not caption_file:
            ...
            samples_all = sample_util.generate_fid_samples(
                ...
                prompts=captions, cfg_scale=target_cfg_scale,
            )
            ...
            des = f"{dataset}/{descriptor}/cfg{target_cfg_scale:.1f}"
